...
@endphp
            ...
            @if (!is_null($sectionData['section']->SectionID))
                <div class="section mb-4 p-4 border rounded bg-light">
                    <h3 class="mb-2">Section {{ $sectionCounter }}: {{ $sectionData['section']->SectionTitle }}</h3>
                    <p class="text-muted">{{ $sectionData['section']->SectionLearningObjective }}</p>
                    @php
                    $sectionCounter += 1; // Khởi tạo biến đếm từ 1
                @endphp
                    <ul class="list-group mb-3">
                        <ul class="list-group mb-3">
                            ...
                            @foreach ($sectionData['curriculums'] as $curriculum)
                                @if (!is_null($curriculum->CurriculumItemID))
                                    @php
                                        $itemType = '';
                                        $itemLabel = '';
                                    ...
                                    <li class="list-group-item border border-secondary rounded mb-2 p-2">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <p style="font-weight: 600; color:black" class="text-xs mb-1">
                                                {{ $itemLabel }}: {{ $curriculum->CurriculumItemTitle }}</p>
                                            @php
                                                $editRoute = '';
                                                if ($curriculum->CurriculumItemType == 'Q') {
                                                    $editRoute = route(
                                        ...
                                        <p class="mb-1 text-muted">{{ $curriculum->CurriculumItemDescription }}</p>
                                    </li>
                                @endif
                            @endforeach
                    ...
                    <div class="mb-3">
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add-curriculum-modal"
                            data-section-id="{{ $sectionData['section']->SectionID }}">Add Curriculum</button>
                    </div>
                                    ...
                                    <form id="curriculum-form" method="POST" action="">
                                        ...
                                        <div id="lecture-fields" style="display: none;">
                                            ...
                                            <div id="loading-icon" style="display: none;">
                                                <img src="{{ asset('H8An.gif') }}" alt="Loading" width="50"
                                                    height="50">
                                            </div>
            ...
            <div class="modal fade" id="add-section-modal" tabindex="-1" aria-labelledby="addSectionLabel"
                ...
                <div class="modal-dialog modal-dialog-centered" style="display: flex; margin: auto;">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="POST"
                                action="{{ route('instructor.courses.section.store', ['courseId' => $courseId]) }}">
                                @csrf
                                <div class="mb-3">
